...
import { categories, seedProducts } from '../../frontend/src/data/grocerySeed.ts'

// A real store starts with owner-entered stock. Samples require an explicit flag.
const includeDemoCatalog = process.argv.includes('--demo-catalog')
...
async function bootstrap(role: 'owner' | 'staff') {
  ...
  if (!email || !password || password.length < 8 || !name)
    throw new Error(prefix + ' email, name and a password of at least 8 characters are required.')
  ...
  await db.profile.upsert({
    ...
    update: { name, role, requestedRole: null, isActive: true },
  })
...
try {
  ...
  for (const seed of includeDemoCatalog ? seedProducts : []) {
    if (await db.product.findUnique({ where: { id: seed.id } })) continue
    const imageName = seed.imageUrl.replace('/images/', '')
    const image = await readFile(resolve(frontendDir, 'public' + seed.imageUrl))
    const contentType = (
      {
        '.jpg': 'image/jpeg',
        '.jpeg': 'image/jpeg',
    ...
    )[extname(seed.imageUrl)]
    if (!contentType) throw new Error('Unsupported sample image format.')
    const { error: uploadError } = await supabaseAdmin.storage
      .from(bucket)
      .upload(imageName, image, { contentType, upsert: false })
  ...
  await db.storeSettings.upsert({
    ...
    create: {
      id: 'store',
      name: 'ASCENT',
      address: 'Store address, Philippines',
    },
    update: {},
  })
  console.log(
    includeDemoCatalog
      ? 'Demo catalog seed complete. Existing products and stock were preserved.'
      : 'Store setup complete. No sample products or stock were inserted. Existing records were preserved; the owner can add inventory.',
  )
} catch (error) {
