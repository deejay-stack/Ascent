...
import { saleInput, items, mergeItems, totals, productInput } from '../src/validation.mjs'

test('sale input rejects invalid quantities, negative amounts and unrecognized methods', () => {
  const sale = {
    transactionId: 'ASC-test-123',
    items: [{ productId: 'rice', quantity: 1 }],
    discount: 0,
    taxRate: 0,
    paymentMethod: 'cash',
    amountReceived: 50,
  ...
  for (const quantity of [0, -1, 0.5, Infinity, 1000001])
    assert.equal(
      saleInput.safeParse({ ...sale, items: [{ productId: 'rice', quantity }] }).success,
      false,
    )
  for (const patch of [
    { amountReceived: -1 },
    { discount: NaN },
    { taxRate: 101 },
    { paymentMethod: 'approved-by-client' },
    { items: [] },
  ])
    assert.equal(saleInput.safeParse({ ...sale, ...patch }).success, false)
...
test('money calculation rounds item prices and tax in cents, including full discount', () => {
  assert.deepEqual(
    ...
    { subtotal: 0.5, discount: 0, tax: 0.06, total: 0.56 },
  )
  assert.deepEqual(totals([{ price: 99.99, quantity: 2 }], 199.98, 12), {
    subtotal: 199.98,
    discount: 199.98,
    tax: 0,
    total: 0,
...
test('duplicate cart rows merge in a stable lock order', () => {
  assert.deepEqual(
    mergeItems([
      ...
      { productId: 'z', quantity: 3 },
    ]),
    [
      { productId: 'a', quantity: 1 },
      { productId: 'z', quantity: 5 },
    ],
  )
})
