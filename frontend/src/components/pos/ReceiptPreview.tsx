...
export function ReceiptPreview({
  ...
  const receiptLabel = !isSupabaseMode
    ? 'DEVELOPMENT RECEIPT'
    : sale.paymentMethod === 'cash'
      ? 'SALES RECEIPT'
      : 'DEMONSTRATION PAYMENT RECEIPT'
  const defaults = settingsService.get()
  const settings = {
    name: sale.storeName ?? defaults.name,
    address: sale.storeAddress ?? defaults.address,
  }
  const save = () => {
    const text = [
      settings.name,
      ...
      ...sale.items.map(
        (l) => `${l.quantity} × ${l.name} @ ${money(l.unitPrice)} = ${money(l.total)}`,
      ),
      ...
      'Thank you for shopping with ASCENT!',
    ].join('\n')
    const url = URL.createObjectURL(new Blob([text], { type: 'text/plain;charset=utf-8' }))
    const link = document.createElement('a')
    link.href = url
    link.download = `${sale.receiptNumber}.txt`
    link.click()
    setTimeout(() => URL.revokeObjectURL(url), 1000)
  }
  return (
    <Modal title="Receipt" className="receipt-modal" onClose={onClose}>
      <article className="receipt-paper" id="print-receipt">
        <header>
          <h2>{settings.name}</h2>
          <p>{settings.address}</p>
          <small>{receiptLabel}</small>
        </header>
        <dl>
          ...
          <dd>{sale.id}</dd>
          <dt>Date / time</dt>
          <dd>{new Date(sale.createdAt).toLocaleString('en-PH')}</dd>
          <dt>Cashier / Owner</dt>
          <dd>{sale.cashier}</dd>
        </dl>
        <table>
        ...
        <p>
          ...
          {sale.paymentMethod !== 'cash' ? ' (simulated)' : ''}
        </p>
        <footer>
          <p>Thank you for shopping with ASCENT!</p>
          <small>Keep your receipt number for order enquiries.</small>
        </footer>
      </article>
