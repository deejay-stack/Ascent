...
export const mockInventoryService: InventoryService = {
  ...
  movements(actor) {
    requireOperator(actor)
    return getState().movements.toReversed()
  },
  async adjust(input) {
    ...
    if (
      ...
      input.quantity < (input.type === 'adjustment' ? 0 : 1)
    )
    ...
    mutate((draft) => {
      ...
      const after =
        input.type === 'adjustment'
          ? input.quantity
          : before + input.quantity * (input.type === 'stock-in' ? 1 : -1)
  ...
  async updateProduct(productId, fields, actor) {
    ...
    cents(fields.sellingPrice)
    cents(fields.costPrice)
    if (!Number.isSafeInteger(fields.lowStockThreshold) || fields.lowStockThreshold < 0)
