...
const out = path.resolve(projectDir, 'artifacts/phase-1')
await mkdir(out, { recursive: true })
const helpers = []
if (!process.env.ASCENT_TEST_URL) {
  helpers.push(
    spawn(process.execPath, [path.join(projectDir, 'backend/src/index.mjs')], {
      env: {
        ...process.env,
...
try {
  ...
  const serviceChecks = await evaluate(
    `import('/tests/commerce-checks.mjs').then(m=>m.runCommerceChecks())`,
  )
  ...
  await field('Choose a supplied photo', '/images/products/staples/fresh_eggs.jpeg')
  await until(
    'document.querySelector(".editor-image").complete && document.querySelector(".editor-image").naturalWidth > 0',
  )
  check(
    'Owner can preview a supplied product photograph',
    await evaluate('document.querySelector(".editor-image").src.endsWith("/fresh_eggs.jpeg")'),
  )
  ...
  check(
    ...
    await evaluate(
      `(async()=>{const m=await import('/src/services/index.ts');const {authApi}=await import('/src/services/api.ts');const actor=await authApi.session();return m.inventoryService.list(actor).some(p=>p.name==='Browser Test Grocery'&&p.stockQuantity===9&&p.imageUrl.endsWith('/fresh_eggs.jpeg'))&&m.inventoryService.movements(actor).some(m=>m.productName==='Browser Test Grocery'&&m.quantity===9)})()`,
    ),
  )
  ...
  await click('[aria-label="Close Filter products"]')
  // Simulate the future empty store in this isolated test profile only.
  await evaluate(
    `localStorage.setItem('ascent-test-catalog-fixture', localStorage.getItem('ascent-commerce-v1'))`,
  )
  await evaluate(
    `import('/src/services/mockStore.ts').then(m=>m.mutate(d=>{d.products=[];d.categories=[];d.sales=[];d.orders=[];d.movements=[]}))`,
  )
  await navigate('/products')
  await until('document.body.textContent.includes("The store is preparing its catalog")')
  check('Empty customer catalog explains that products are being added', true)
  await navigate('/')
  await until('!!document.querySelector(".home-page")')
  check(
    'Empty homepage does not show stale categories or featured products',
    await evaluate(
      '!document.querySelector(".category-card")&&!document.querySelector(".featured-section")',
    ),
  )
  await navigate('/owner/inventory')
  await until('document.body.textContent.includes("Your inventory is empty")')
  await button('Manage categories')
  await field('Category name', 'Owner first aisle')
  await button('Add category')
  await until('document.querySelector("dialog").textContent.includes("Owner first aisle")')
  await click('[aria-label="Close Manage categories"]')
  await button('Add product')
  await field('Product name', 'Owner First Product')
  await field('Description', 'The first item in an empty store.')
  await field('Selling price', '25')
  await field('Cost price', '15')
  await field('SKU', 'FIRST-001')
  await field('Barcode', '1234567890128')
  await field('Opening stock', '5')
  await field('Choose a supplied photo', '/images/products/produce/banana.jpg')
  await button('Create product')
  await until('!document.querySelector("dialog[open]")')
  await navigate('/products')
  await until('document.querySelectorAll(".catalog-card").length===1')
  check(
    'Owner can populate an empty inventory and publish it to the catalog',
    await evaluate(
      'document.querySelector(".catalog-card").textContent.includes("Owner First Product")',
    ),
  )
  await navigate('/')
  await until('document.querySelectorAll(".category-card").length===1')
  await click('.category-card')
  await until('document.querySelectorAll(".catalog-card").length===1')
  check(
    'Owner-created homepage category opens a working catalog filter',
    await evaluate(
      'document.querySelector(".desktop-filters select").value===new URLSearchParams(location.search).get("category")',
    ),
  )
  await evaluate(
    `import('/src/services/mockStore.ts').then(m=>m.mutate(d=>Object.assign(d,JSON.parse(localStorage.getItem('ascent-test-catalog-fixture')))))`,
  )
  await evaluate(
    `(()=>{const state=JSON.parse(localStorage.getItem('ascent-commerce-v1'));state.products.find(p=>p.id==='grocery-001').imageUrl='/images/products/staples/premium-rice-5-kg.svg';localStorage.setItem('ascent-commerce-v1',JSON.stringify(state))})()`,
  )
  await navigate('/owner/inventory')
  await until('!!document.querySelector(".table-product")')
  check(
    'Previously saved catalogs receive photos without losing prices, stock or history',
    await evaluate(
      `(async()=>{const {getState}=await import('/src/services/mockStore.ts');const expected=JSON.parse(localStorage.getItem('ascent-test-catalog-fixture'));return JSON.stringify(getState())===JSON.stringify(expected)})()`,
    ),
  )
  await evaluate(`localStorage.removeItem('ascent-test-catalog-fixture')`)
  await writeFile(
