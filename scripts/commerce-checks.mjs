...
export async function runCommerceChecks() {
  ...
  try {
    ...
    await rejects('Negative quantity rejected', () =>
      salesService.complete({ ...request(), items: [{ productId: p.id, quantity: -1 }] }),
    )
    await rejects('Zero quantity rejected', () =>
      salesService.complete({ ...request(), items: [{ productId: p.id, quantity: 0 }] }),
    )
    await rejects('Over-stock rejected', () =>
      salesService.complete({ ...request(), items: [{ productId: p.id, quantity: p.stock + 1 }] }),
    )
    ...
    for (const p of all) {
      ...
      if (!result.ok || !result.headers.get('content-type')?.includes('svg'))
