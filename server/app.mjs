...
app.get('/api/people', roles('owner'), async (_req, res) =>
  res.json({ users: await db.profile.findMany({ orderBy: { name: 'asc' } }) }),
)
...
app.patch('/api/people/:id', roles('owner'), async (req, res) => {
  ...
  const person = await db.profile.findUnique({ where: { id: req.params.id } })
  if (!person) fail(404, 'Account not found.')
  if (person.role === 'owner') fail(400, 'Owner access cannot be disabled here.')
  res.json({ user: await db.profile.update({ where: { id: person.id }, data: { isActive } }) })
})
