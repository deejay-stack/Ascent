...
const makeAccount = (id, name, email, role, password) => {
  ...
  return { id, name, email: email.toLowerCase(), role, passwordSalt, passwordHash: hashPassword(password, passwordSalt) }
}
...
try {
  ...
  store.products = Array.isArray(store.products) && store.products.length ? store.products : seedProducts
...
const getSession = (req) => {
  ...
  return { token, user: store.users.find((user) => user.id === session.userId) }
}
...
const api = async (req, res, url) => {
  ...
  if (req.method === 'GET' && url.pathname === '/api/products') {
    ...
    return send(res, 200, { products: result, categories: ['All', ...new Set(store.products.map((product) => product.category))] })
  }
  ...
  if (req.method === 'POST' && url.pathname === '/api/auth/login') {
    ...
    if (!user) return send(res, 401, { message: 'Email or password is incorrect.' })
    const supplied = Buffer.from(hashPassword(String(body.password || ''), user.passwordSalt), 'hex')
    const expected = Buffer.from(user.passwordHash, 'hex')
    if (supplied.length !== expected.length || !timingSafeEqual(supplied, expected)) return send(res, 401, { message: 'Email or password is incorrect.' })
  ...
  if (req.method === 'POST' && url.pathname === '/api/auth/register') {
    ...
    if (name.length < 2 || !/^\S+@\S+\.\S+$/.test(email) || password.length < 8) return send(res, 400, { message: 'Enter a valid name, email, and password of at least 8 characters.' })
    if (store.users.some((user) => user.email === email)) return send(res, 409, { message: 'An account with this email already exists.' })
  ...
  if (req.method === 'POST' && url.pathname === '/api/auth/recover') {
    await readBody(req)
    return send(res, 200, { message: 'If an account exists, reset instructions have been prepared.' })
  }
  ...
  if (req.method === 'GET' && url.pathname === '/api/orders') {
    ...
    const orders = session.user.role === 'customer' ? store.orders.filter((order) => order.userId === session.user.id) : store.orders
  ...
  if (req.method === 'POST' && url.pathname === '/api/orders') {
    const session = requireSession(req, res)
    if (!session?.user) return
    if (session.user.role !== 'customer') return send(res, 403, { message: 'Checkout is available to customer accounts.' })
    const body = await readBody(req)
    ...
    for (const item of requested) {
      ...
      if (!product || !quantity) return send(res, 400, { message: 'The cart contains an invalid item.' })
      if (product.stock < quantity) return send(res, 409, { message: `${product.name} has only ${product.stock} available.` })
      lines.push({ productId: product.id, name: product.name, price: product.price, unit: product.unit, quantity, lineTotal: product.price * quantity })
    }
    for (const line of lines) store.products.find((product) => product.id === line.productId).stock -= line.quantity
    const subtotal = lines.reduce((sum, line) => sum + line.lineTotal, 0)
    const order = { id: `ASC-${String(Date.now()).slice(-8)}`, userId: session.user.id, status: 'Processing', fulfillment: body.fulfillment === 'delivery' ? 'Delivery' : 'Store pickup', createdAt: new Date().toISOString(), items: lines, subtotal, total: subtotal }
...
const mimeTypes = { '.html': 'text/html; charset=utf-8', '.js': 'text/javascript; charset=utf-8', '.css': 'text/css; charset=utf-8', '.svg': 'image/svg+xml', '.png': 'image/png', '.jpg': 'image/jpeg', '.ico': 'image/x-icon' }
const serveApp = (res, pathname) => {
  ...
  if (!existsSync(file)) return send(res, 404, { message: 'Build the frontend with npm run build first.' })
...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    console.error(error)
    return send(res, error instanceof SyntaxError ? 400 : 500, { message: error instanceof Error ? error.message : 'Unexpected server error.' })
  }
})
