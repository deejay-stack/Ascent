  ...
  return (
    <aside className="billing-panel">
            ...
            if (!product)
              return (
                <div key={item.productId}>
                  Product unavailable{' '}
                  <button onClick={() => p.onRemove(item.productId)}>Remove item</button>
                </div>
              )
            return (
      ...
      <label className="field">
        Payment method
        <select value={p.method} onChange={(e) => p.onMethod(e.target.value as PaymentMethod)}>
          <option value="cash">Cash</option>
          <option value="gcash">GCash · demonstration</option>
          <option value="maya">Maya · demonstration</option>
          <option value="card">Card · demonstration</option>
        </select>
      </label>
      {p.method === 'cash' ? (
        <>
          ...
          <button className="text-action" onClick={() => p.onReceived(p.totals.total.toFixed(2))}>
            Exact amount
          </button>
      ...
      <p className="muted small">
        Temporary local transactions · Tax is a configurable demo amount.
      </p>
    </aside>
  )
}
