export const cents = (amount: number) => {
  if (!Number.isFinite(amount) || amount < 0 || amount > 100_000_000) throw new Error('Enter a valid non-negative amount.')
  return Math.round((amount + Number.EPSILON) * 100)
}
export const money = (amount: number) => new Intl.NumberFormat('en-PH', { style: 'currency', currency: 'PHP' }).format(amount)
export function calculateTotals(lines: { price: number; quantity: number }[], discount = 0, taxRate = 0) {
  ...
  return { subtotal: subtotal / 100, discount: discountCents / 100, tax: tax / 100, total: (subtotal - discountCents + tax) / 100 }
}
