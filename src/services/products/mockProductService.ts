import { getState, getRevision, projectCatalog, subscribe } from '../mockStore'
import type { ProductQuery, ProductService } from './productService'

function snapshot(query: ProductQuery = {}) {
  const search = query.search?.trim().toLowerCase() ?? ''
  let products = projectCatalog().filter(
    (p) =>
      (!search ||
        [p.name, p.sku, p.barcode, p.brand, p.category].join(' ').toLowerCase().includes(search)) &&
      (!query.category ||
        query.category === 'All' ||
...
export const mockProductService: ProductService = {
  snapshot,
  categories: () => getState().categories.map((c) => ({ ...c })),
  subscribe,
  revision: getRevision,
  async list(query) {
    return {
      products: snapshot(query),
      categories: ['All', ...getState().categories.map((c) => c.name)],
    }
  },
  async get(id) {
    const product = snapshot().find((p) => p.id === id || p.slug === id)
